...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT room_name FROM Room WHERE ";
    $conditions = array();
    $parameters = array(); // Initialize the parameters array

    // Add the conditions based on the filter values
    if ($roomType !== '') {
        $conditions[] = "room_name = :roomType";
        $parameters[':roomType'] = $roomType; // Add parameter value
    }
    ...
    if ($occupancy !== '') {
        ...
        $parameters[':occupancy'] = $occupancy; // Add parameter value
    }
...
// Bind the parameter values, if applicable
...
// Fetch the room names
$roomNames = $stmt->fetchAll(PDO::FETCH_COLUMN);

// Output the filtered room names
if (!empty($roomNames)) {
    echo "<h2>Filtered Room Names:</h2>";
    echo "<ul>";
...
<form method="POST">
    ...
    <select id="room-type" name="room-type"> <!-- Updated id and added name attribute -->
...
<select id="bed-type" name="bed-type"> <!-- Updated id and added name attribute -->
    <option value="">Any</option>
...
<select id="occupancy" name="occupancy"> <!-- Updated id and added name attribute -->
    <option value="">Any</option>
...
<select id="price-range" name="price-range"> <!-- Updated id and added name attribute -->
    <option value="">Any</option>
